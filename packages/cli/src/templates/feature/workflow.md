...
# Example: python3 ./.feature/scripts/init_developer.py cursor-agent
```

This creates:
- `.feature/.developer` - Your identity file (gitignored, not committed)
- `.feature/workspace/<your-name>/` - Your personal workspace directory
...
# Or check manually:
python3 ./.feature/scripts/get_developer.py      # Your identity
python3 ./.feature/scripts/task.py list          # Active tasks
...
**Frontend Task**:
```bash
cat .feature/spec/frontend/hook-guidelines.md      # For hooks
cat .feature/spec/frontend/component-guidelines.md # For components
cat .feature/spec/frontend/type-safety.md          # For types
```

**Backend Task**:
```bash
cat .feature/spec/backend/database-guidelines.md   # For DB operations
cat .feature/spec/backend/type-safety.md           # For types
cat .feature/spec/backend/logging-guidelines.md    # For logging
```
...
2. **Follow Standards** - [!] **MUST read `.feature/spec/` guidelines before coding**
...
# Or get JSON format
python3 ./.feature/scripts/get_context.py --json
```
...
cat .feature/spec/guides/cross-layer-thinking-guide.md
```
...
python3 ./.feature/scripts/task.py create "<title>" --slug <task-name>
```
...
1. Create or select task
   --> python3 ./.feature/scripts/task.py create "<title>" --slug <name> or list

2. Write code according to guidelines
   --> Read .feature/spec/ docs relevant to your task
   --> For cross-layer: read .feature/spec/guides/

3. Generate test cases (for complex tasks)
   --> /feature:write-testcase: Generate test cases based on task documents
   --> /feature:check-testcase: Verify test case completeness

4. Self-test
   --> Run project's lint/test commands (see spec docs)
   --> Manual feature testing
   --> Verify against test cases (if generated)
...
6. Record session (one command)
   --> python3 ./.feature/scripts/add_session.py --title "Title" --commit "hash"
```

### Complex Task Workflow (Mandatory Steps)

For complex tasks involving multiple modules or architectural changes:

```
write-plan -> write-testcase -> check-testcase -> executing-plans
```

**Test Case Commands**:
- `/feature:write-testcase` - Generate standardized test cases from task documents
- `/feature:check-testcase` - Verify test case completeness against product checklist

**Output Files**:
- `testcase_analysis.md` - Requirements analysis report
- `testcase.md` - Test case document
- `testcase_checkdetail.md` - Test case verification report
...
- [OK] Test cases verified (for complex tasks)

**Project-specific checks**:
- See `.feature/spec/frontend/quality-guidelines.md` for frontend
- See `.feature/spec/backend/quality-guidelines.md` for backend
...
Use `/feature:finish-work` command to run through:
...
|   |-- testcase_analysis.md    # For complex tasks
|   |-- testcase.md             # For complex tasks
|   +-- testcase_checkdetail.md # For complex tasks
+-- archive/
    +-- 2026-01/
        +-- 01-15-old-task/
            +-- task.json
...
python3 ./.feature/scripts/task.py archive <name>  # Archive to archive/{year-month}/
python3 ./.feature/scripts/task.py list            # List active tasks
python3 ./.feature/scripts/task.py list-archive    # List archived tasks
```
...
1. **Before session start**:
   - Run `python3 ./.feature/scripts/get_context.py` for full context
   - [!] **MUST read** relevant `.feature/spec/` docs

2. **During development**:
   - [!] **Follow** `.feature/spec/` guidelines
   - For complex tasks, run `/feature:write-testcase` and `/feature:check-testcase`
   - For cross-layer features, use `/feature:check-cross-layer`
   - Develop only one task at a time
   - Run lint and tests frequently

3. **After development complete**:
   - Use `/feature:finish-work` for completion checklist
   - After fix bug, use `/feature:break-loop` for deep analysis
   - Human commits after testing passes
   - Use `add_session.py` to record progress

### [X] DON'T - Should Not Do

1. [!] **Don't** skip reading `.feature/spec/` guidelines
...
| Complex Task | Generate test cases via `/feature:write-testcase` |
...
```bash
# Session management
python3 ./.feature/scripts/get_context.py    # Get full context
python3 ./.feature/scripts/add_session.py    # Record session

# Task management
python3 ./.feature/scripts/task.py list      # List tasks
python3 ./.feature/scripts/task.py create "<title>" # Create task

# Slash commands
/feature:write-testcase       # Generate test cases (complex tasks)
/feature:check-testcase       # Verify test case completeness
/feature:finish-work          # Pre-commit checklist
/feature:break-loop           # Post-debug analysis
