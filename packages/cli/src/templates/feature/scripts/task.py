...
def _resolve_task_dir(target_dir: str, repo_root: Path) -> Path:
    ...
    - Relative path: .feature/tasks/01-31-my-task
    ...
    # Relative path (contains path separator or starts with .feature)
    if "/" in target_dir or target_dir.startswith(".feature"):
...
def get_check_context(dev_type: str, repo_root: Path) -> list[dict]:
    ...
    entries = [
        {"file": adapter.get_feature_command_path("finish-work"), "reason": "Finish work checklist"},
    ]

    if dev_type in ("backend", "fullstack"):
        entries.append({"file": adapter.get_feature_command_path("check-backend"), "reason": "Backend check spec"})
    if dev_type in ("frontend", "fullstack"):
        entries.append({"file": adapter.get_feature_command_path("check-frontend"), "reason": "Frontend check spec"})
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not full_path.is_dir():
        ...
        print("Hint: Use task name (e.g., 'my-task') or full path (e.g., '.feature/tasks/01-31-my-task')")
        return 1
...
def _auto_commit_archive(task_name: str, repo_root: Path) -> None:
    """Stage .feature/tasks/ changes and commit after archive."""
...
def cmd_set_branch(args: argparse.Namespace) -> int:
    ...
    print(f"  python3 ./.feature/scripts/multi_agent/start.py {args.dir}")
    return 0
...
Examples:
  ...
  python3 task.py create "Child task" --slug child --parent .feature/tasks/01-21-parent
  python3 task.py init-context .feature/tasks/01-21-add-login backend
  python3 task.py add-context <dir> implement .feature/spec/backend/auth.md "Auth guidelines"
  python3 task.py set-branch <dir> task/add-login
  python3 task.py start .feature/tasks/01-21-add-login
