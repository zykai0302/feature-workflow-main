...
def find_task_log(search: str, repo_root: Path) -> Path | None:
    """Find task log file by task name (supports Plan Agent logs).

    Looks for:
    1. .feature/tasks/<search>/.plan-log (Plan Agent log)
    2. .feature/tasks/*<search>*/.plan-log (partial match)
    ...
    # Exact match
    exact_path = tasks_dir / search / ".plan-log"
    if exact_path.is_file():
        return exact_path

    # Partial match
    for task_dir in tasks_dir.iterdir():
        if task_dir.is_dir() and search in task_dir.name:
            log_file = task_dir / ".plan-log"
            if log_file.is_file():
                return log_file
...
def find_task_dir(search: str, repo_root: Path) -> Path | None:
    """Find task directory by task name.
...
def cmd_summary(repo_root: Path, filter_assignee: str | None = None) -> int:
    ...
    if stopped_tasks:
        ...
        for t in stopped_tasks:
            platform = t.get("platform", "claude")
            ...
            if t["status"] == "completed":
                ...
                # Try to get session ID from file
                session_id = None
                ...
                # Try to extract session ID from log if not found
                if not session_id and t["log_file"].is_file():
                    try:
                        log_content = t["log_file"].read_text(encoding="utf-8", errors="replace")
                        session_id = adapter.extract_session_id_from_log(log_content)
                        if session_id:
                            # Save it for future use
                            t["session_id_file"].write_text(session_id, encoding="utf-8")
                    except Exception:
                        pass

                # Get last message/activity for context
                last_msg = get_last_message(t["log_file"], 150, platform=platform)
                if last_msg:
                    print(f'{Colors.DIM}  Last: "{last_msg}"{Colors.NC}')
                ...
                if session_id:
                    ...
                    print(f"  {Colors.DIM}(session ID not found in logs){Colors.NC}")
                    print(f"  {Colors.YELLOW}Tip:{Colors.NC} Check log file for session ID:")
                    print(f"       {t['log_file']}")
            print()
...
def cmd_watch(target: str, repo_root: Path) -> int:
    ...
    # First try to find agent in registry
    agent = find_agent(target, repo_root)

    if agent:
        # Dispatch agent log (from worktree)
        worktree = agent.get("worktree_path", "")
        log_file = Path(worktree) / ".agent-log"
        task_name = target
    else:
        # Try to find Plan Agent log
        log_file = find_task_log(target, repo_root)
        if not log_file:
            print(f"Agent not found: {target}")
            print()
            print(f"{Colors.YELLOW}Tip: For Plan Agent logs, use the task name:{Colors.NC}")
            print(f"     python ./.feature/scripts/multi_agent/status.py --watch <task-name>")
            print()
            print(f"{Colors.DIM}Available tasks with logs:{Colors.NC}")
            tasks_dir = get_tasks_dir(repo_root)
            if tasks_dir.is_dir():
                for task_dir in sorted(tasks_dir.iterdir()):
                    if task_dir.is_dir() and not task_dir.name.startswith("."):
                        plan_log = task_dir / ".plan-log"
                        if plan_log.is_file():
    ...
    print(f"{Colors.DIM}Task: {task_name}{Colors.NC}")
...
def cmd_log(target: str, repo_root: Path) -> int:
    ...
    if agent:
        ...
        if not log_file:
            ...
            print()
            print(f"{Colors.YELLOW}Tip: For Plan Agent logs, use the task name:{Colors.NC}")
            print(f"     python ./.feature/scripts/multi_agent/status.py --log <task-name>")
            print()
            print(f"{Colors.DIM}Available tasks:{Colors.NC}")
            tasks_dir = get_tasks_dir(repo_root)
            if tasks_dir.is_dir():
                for task_dir in sorted(tasks_dir.iterdir()):
                    if task_dir.is_dir() and not task_dir.name.startswith("."):
                        plan_log = task_dir / ".plan-log"
                        has_plan = "✓" if plan_log.is_file() else " "
                        print(f"  [{has_plan}] {task_dir.name}")
            return 1
        platform = "claude"  # Default platform for plan logs
        task_name = log_file.parent.name
    ...
    print(f"{Colors.BLUE}=== Recent Log: {task_name} ==={Colors.NC}")
    print(f"{Colors.DIM}Platform: {platform}{Colors.NC}")
    print(f"{Colors.DIM}Log file: {log_file}{Colors.NC}")
    print()
