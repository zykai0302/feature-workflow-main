...
Example: python3 start.py .feature/tasks/01-21-my-task
...
from common.cli_adapter import CLIAdapter, get_cli_adapter, detect_platform
from common.git_context import _run_git_command, _is_git_repo, _ensure_git_repo
...
def _read_json_file(path: Path, verbose: bool = False) -> dict | None:
    ...
    try:
        ...
        if verbose:
            ...
            log_error(f"  Line {e.lineno}, Column {e.colno}: {e.msg}")
            # Show the problematic line
            try:
                lines = path.read_text(encoding="utf-8").splitlines()
                if e.lineno and e.lineno <= len(lines):
                    log_error(f"  Line {e.lineno}: {lines[e.lineno - 1][:100]}")
            except Exception:
                pass
        return None
    except OSError:
...
# No default platform - will auto-detect
...
def main() -> int:
    ...
    parser.add_argument(
        "--platform", "-p",
        choices=["claude", "cursor", "iflow", "opencode", "qoder", "codebuddy"],
        default=None,
    ...
    parser.add_argument(
        "--model", "-m",
        default="glm-5.0",
        help="Model to use (CodeBuddy only, e.g., glm-4.7, glm-5.0)"
    )
    ...
    # Get repo root early for platform detection
    ...
    if platform is None:
        ...
        log_info(f"Auto-detected platform: {platform}")
    ...
    task_data = _read_json_file(task_json_path, verbose=True)
    if not task_data:
        log_error("Failed to read task.json - check for JSON syntax errors above")
        return 1
    ...
    # CRITICAL: Must use Task tool to call subagents, NOT do implementation work directly
    cli_cmd = adapter.build_run_command(
        agent="dispatch",
        prompt="""You are the Dispatch Agent. Your ONLY job is to call subagents using the Task tool.

CRITICAL INSTRUCTIONS:
1. Read .feature/.current-task to get task directory
2. Read task.json from that directory
3. For EACH action in next_action array, you MUST call Task tool:
   - implement: Task(subagent_name="implement", prompt="Implement the feature")
   - check: Task(subagent_name="check", prompt="Check code changes")
   - finish: Task(subagent_name="check", prompt="[finish] Final check")
   - create-pr: Run python ./.feature/scripts/multi_agent/create_pr.py

NEVER do implementation work yourself. ONLY call Task tool for each phase.

Execute ALL phases in order without stopping.""",
    ...
    with log_file.open("w") as log_f:
        # On Windows, use shell=True to resolve npm global commands
        ...
        if sys.platform == "win32":
            # Windows: use shell=True to resolve npm global commands like 'codebuddy'
            popen_kwargs["shell"] = True
            popen_kwargs["creationflags"] = subprocess.CREATE_NEW_PROCESS_GROUP
            # Convert list to string for shell=True
            cli_cmd_str = " ".join(f'"{arg}"' if " " in arg else arg for arg in cli_cmd)
            process = subprocess.Popen(cli_cmd_str, **popen_kwargs)
        else:
    ...
    # For OpenCode/CodeBuddy: extract session ID from logs after startup
    if not adapter.supports_session_id_on_create:
        import time
        log_info("Waiting for session ID from logs...")
        # Wait a bit for the log to have session ID (longer for CodeBuddy)
        max_attempts = 20 if adapter.platform == "codebuddy" else 10
        for _ in range(max_attempts):
            ...
            log_info("Session ID will be extracted later when running status.py")
            session_id = None  # Don't set to "unknown", let status.py try again
    ...
    print()
    print(f"{Colors.YELLOW}To monitor:{Colors.NC} ", end="")
    if sys.platform == "win32":
        print(f"Get-Content -Wait {log_file}  # PowerShell")
        print(f"             type {log_file}  # CMD (one-time)")
    else:
        print(f"tail -f {log_file}")
    print(f"{Colors.YELLOW}To stop:{Colors.NC}    ", end="")
    if sys.platform == "win32":
        print(f"taskkill /PID {agent_pid} /F")
    else:
        print(f"kill {agent_pid}")
    if session_id:
        ...
        print(f"{Colors.YELLOW}To resume:{Colors.NC}  Run status.py after agent starts to get session ID")
