...
from common.cli_adapter import get_cli_adapter, detect_platform
...
def main() -> int:
    ...
    cli_cmd = adapter.build_run_command(
        agent="plan",  # Will be mapped to "feature-plan" for OpenCode
    ...
    if sys.platform == "win32":
        print(f"  tasklist /FI \"PID eq {agent_pid}\"")
    else:
        print(f"  ps -p {agent_pid}")
    print(f"  dir {task_dir}")
    print()
    print(f"{Colors.YELLOW}After completion, run:{Colors.NC}")
    print(f"  python3 ./.feature/scripts/multi_agent/start.py {task_dir}")
