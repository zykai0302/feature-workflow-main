...
def _is_git_repo(cwd: Path | None = None) -> bool:
    ...
    ret, _, _ = _run_git_command(["rev-parse", "--is-inside-work-tree"], cwd=cwd)
    return ret == 0


def _init_git_repo(cwd: Path | None = None) -> tuple[int, str, str]:
    """Initialize a git repository if it doesn't exist."""
    if _is_git_repo(cwd):
        return 0, "Already a git repository", ""
    
    ret, out, err = _run_git_command(["init"], cwd=cwd)
    if ret == 0:
        # Create initial commit to allow branching
        _run_git_command(["config", "user.email", "feature@example.com"], cwd=cwd)
        _run_git_command(["config", "user.name", "feature"], cwd=cwd)
        # Create .gitignore if not exists
        gitignore = (cwd or Path.cwd()) / ".gitignore"
        if not gitignore.exists():
            gitignore.write_text("# feature workspace\nfeature-worktrees/\n*.log\n.agent-log\n.session-id\n")
    return ret, out, err


def _ensure_git_repo(cwd: Path | None = None) -> bool:
    """Ensure git repo exists, initialize if needed. Returns True if git is ready."""
    if _is_git_repo(cwd):
        return True
    ret, _, err = _init_git_repo(cwd)
    if ret != 0:
        return False
