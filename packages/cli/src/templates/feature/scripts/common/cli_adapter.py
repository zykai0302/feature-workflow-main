...
class CLIAdapter:
    ...
    platform: Platform
    default_model: str | None = "glm-5.0"  # Default model for CodeBuddy resume commands
    ...
    _AGENT_NAME_MAP: ClassVar[dict[Platform, dict[str, str]]] = {
        ...
        "opencode": {
            "plan": "feature-plan",  # 'plan' is built-in in OpenCode
        },
    }
    ...
    def get_agent_name(self, agent: str) -> str:
        ...
        Returns:
            Platform-specific agent name (e.g., 'feature-plan' for OpenCode)
        """
    ...
    def get_commands_path(self, project_root: Path, *parts: str) -> Path:
        ...
        Args:
            project_root: Project root directory
            *parts: Additional path parts (e.g., 'feature', 'finish-work.md')

        Returns:
            Path to commands directory or file

        Note:
            Cursor uses prefix naming: .cursor/commands/feature-<name>.md
            Antigravity uses workflow directory: .agent/workflows/<name>.md
            Claude/OpenCode use subdirectory: .claude/commands/feature/<name>.md
        """
        ...
        if self.platform == "cursor" and len(parts) >= 2 and parts[0] == "feature":
            # Convert feature/<name>.md to feature-<name>.md
            filename = parts[-1]
            return (
                self.get_config_dir(project_root) / "commands" / f"feature-{filename}"
            )

        return self.get_config_dir(project_root) / "commands" / Path(*parts)

    def get_feature_command_path(self, name: str) -> str:
        """Get relative path to a feature command file.
        ...
        Note:
            ...
            Gemini: .gemini/commands/feature/<name>.toml
            Antigravity: .agent/workflows/<name>.md
            Others: .{platform}/commands/feature/<name>.md
        """
        if self.platform == "cursor":
            return f".cursor/commands/feature-{name}.md"
        elif self.platform == "codex":
            return f".agents/skills/{name}/SKILL.md"
        elif self.platform == "kiro":
            return f".kiro/skills/{name}/SKILL.md"
        elif self.platform == "gemini":
            return f".gemini/commands/feature/{name}.toml"
        elif self.platform == "antigravity":
            return f".agent/workflows/{name}.md"
        elif self.platform == "kilo":
            return f".kilocode/workflows/{name}.md"
        else:
            return f"{self.config_dir_name}/commands/feature/{name}.md"
    ...
    def get_non_interactive_env(self) -> dict[str, str]:
        ...
        if self.platform == "opencode":
            ...
            return {"CODEBUDDY_NON_INTERACTIVE": "1"}
        else:
    ...
    def build_run_command(
        ...
        Args:
            ...
            session_id: Optional session ID (Claude Code and CodeBuddy only for creation)
            skip_permissions: Whether to skip permission prompts
            verbose: Whether to enable verbose output
            json_output: Whether to use JSON output format
            model: Optional model to use (CodeBuddy only, e.g., "glm-4.7", "glm-5.0")
        ...
        if self.platform == "opencode":
            ...
            # CodeBuddy CLI auto-loads agents from .codebuddy/agents/ directory
            # No --agent parameter needed, just use system prompt to guide behavior
            # See: codebuddy --help
            cmd = ["codebuddy", "-p"]
            ...
            # For agents, use system-prompt to activate the agent behavior
            if agent:
                agent_prompt = f"You are the {agent} agent. Follow your agent definition and execute the task. "
                cmd.append(agent_prompt + prompt)
            else:
                cmd.append(prompt)
    ...
    def build_resume_command(self, session_id: str) -> list[str]:
        ...
        if self.platform == "opencode":
            ...
            # CodeBuddy uses -r or --resume for session resumption
            cmd = ["codebuddy", "-r", session_id]
    ...
    def supports_cli_agents(self) -> bool:
        ...
        Claude Code, OpenCode, iFlow, and CodeBuddy support CLI agent execution.
        Cursor is IDE-only and doesn't support CLI agents.
        """
        return self.platform in ("claude", "opencode", "iflow", "codebuddy")
    ...
    def extract_session_id_from_log(self, log_content: str) -> str | None:
        ...
        if self.platform == "opencode":
            # OpenCode session ID pattern: ses_xxx
            match = re.search(r"ses_[a-zA-Z0-9]+", log_content)
            if match:
                return match.group(0)
        elif self.platform == "codebuddy":
            # CodeBuddy session ID is a UUID
            # Look for patterns like "session id: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
            # or "sessionId":"xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
            patterns = [
                r'session[_-]?id["\s:]+([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})',
            ...
            # Also try to find any UUID that looks like a session ID
            match = re.search(r'\b([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})\b', log_content, re.IGNORECASE)
            if match:
                return match.group(1)
        return None
...
def get_cli_adapter(platform: str = "claude", default_model: str | None = None) -> CLIAdapter:
    """Get CLI adapter for the specified platform.

    Args:
        platform: Platform name ('claude', 'opencode', 'cursor', 'iflow', 'codex', 'kilo', 'kiro', 'gemini', 'antigravity', 'qoder', or 'codebuddy')
        default_model: Default model to use for resume commands (e.g., "glm-5.0" for CodeBuddy)
    ...
    return CLIAdapter(platform=platform, default_model=default_model)  # type: ignore
...
def detect_platform(project_root: Path) -> Platform:
    ...
    Detection order:
    1. feature_PLATFORM environment variable (if set)
    ...
    Returns:
        Detected platform ('claude', 'opencode', 'cursor', 'iflow', 'codex', 'kilo', 'kiro', 'gemini', 'antigravity', 'qoder', or 'codebuddy')
    """
    import os

    # Check environment variable first
    env_platform = os.environ.get("feature_PLATFORM", "").lower()
    if env_platform in (
        "claude",
        "opencode",
        "cursor",
