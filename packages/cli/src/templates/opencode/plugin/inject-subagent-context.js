/**
 * feature Context Injection Plugin
 *
...
import { featureContext, debugLog } from "../lib/feature-context.js"
...
function getCheckContext(ctx, taskDir) {
  ...
  if (entries.length > 0) {
    ...
    const checkFiles = [
      [".opencode/commands/feature/finish-work.md", "Finish work checklist"],
      [".opencode/commands/feature/check-cross-layer.md", "Cross-layer check spec"],
      [".opencode/commands/feature/check-backend.md", "Backend check spec"],
      [".opencode/commands/feature/check-frontend.md", "Frontend check spec"],
    ]
...
function getFinishContext(ctx, taskDir) {
  ...
  if (entries.length > 0) {
    ...
    const finishWork = ctx.readProjectFile(".opencode/commands/feature/finish-work.md")
    if (finishWork) {
      parts.push(`=== .opencode/commands/feature/finish-work.md (Finish checklist) ===\n${finishWork}`)
    }
  }

  // 2. Spec update process (for active spec sync)
  const updateSpec = ctx.readProjectFile(".opencode/commands/feature/update-spec.md")
  if (updateSpec) {
    parts.push(`=== .opencode/commands/feature/update-spec.md (Spec update process) ===\n${updateSpec}`)
  }
...
.feature/big-question/  # Known issues and pitfalls
\`\`\`

## Search Tips

- Spec files: \`.feature/spec/**/*.md\`
- Known issues: \`.feature/big-question/\`
...
export default async ({ directory }) => {
  const ctx = new featureContext(directory)
  ...
  return {
    ...
    //
    // SOLUTION: feature + OpenCode users must install oh-my-opencode (omo)
