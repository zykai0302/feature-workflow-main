...
import { featureContext, contextCollector, debugLog } from "../lib/feature-context.js"
...
function getTaskStatus(directory) {
  const featureDir = join(directory, ".feature")
  const currentTaskFile = join(featureDir, ".current-task")
  ...
  if (taskRef.startsWith("/")) {
    taskDir = taskRef
  } else if (taskRef.startsWith(".feature/")) {
    taskDir = join(directory, taskRef)
  } else {
    taskDir = join(featureDir, "tasks", taskRef)
  }

  if (!existsSync(taskDir)) {
    return `Status: STALE POINTER\nTask: ${taskRef}\nNext: Task directory not found. Run: python3 ./.feature/scripts/task.py finish`
  }
  ...
  if (taskStatus === "completed") {
    ...
    return `Status: COMPLETED\nTask: ${taskTitle}\nNext: Archive with \`python3 ./.feature/scripts/task.py archive ${dirName}\` or start a new task`
  }
...
You are starting a new session in a feature-managed project.
Read and follow all instructions below carefully.
</feature-context>`)

  // 2. Current Context (dynamic)
  const contextScript = join(featureDir, "scripts", "get_context.py")
  ...
  // 3. Workflow Guide
  const workflow = ctx.readProjectFile(".feature/workflow.md")
  if (workflow) {
    parts.push("<workflow>")
    parts.push(workflow)
    parts.push("</workflow>")
  ...
  const specDir = join(directory, ".feature", "spec")
  ...
  let startMd = ctx.readFile(join(claudeDir, "commands", "feature", "start.md"))
  if (!startMd) {
    startMd = ctx.readFile(join(opencodeDir, "commands", "feature", "start.md"))
  }
  if (startMd) {
