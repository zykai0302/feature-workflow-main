...
// Debug logging
const DEBUG_LOG = "/tmp/feature-plugin-debug.log"
...
export class featureContext {
  constructor(directory) {
    ...
    debugLog("context", "featureContext initialized", { directory })
  }
  ...
  /**
   * Check if this is a feature-managed project
   */
  isfeatureProject() {
    return existsSync(join(this.directory, ".feature"))
  }

  /**
   * Get current task directory from .feature/.current-task
   */
  getCurrentTask() {
    try {
      const currentTaskPath = join(this.directory, ".feature", ".current-task")
  ...
  shouldSkipHook(hookName) {
    // Not a feature project? Skip.
    if (!this.isfeatureProject()) {
      debugLog("context", `shouldSkipHook(${hookName}): skip - not feature project`)
      return true
    }
