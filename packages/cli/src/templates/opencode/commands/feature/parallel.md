...
### Step 1: Understand feature Workflow `[AI]`
...
```bash
cat .feature/spec/frontend/index.md  # Frontend guidelines index
cat .feature/spec/backend/index.md   # Backend guidelines index
...
```bash
python3 ./.feature/scripts/multi_agent/start.py "$TASK_DIR" --platform opencode
```
...
TASK_DIR=$(python3 ./.feature/scripts/task.py create "<title>" --slug <task-name>)
```
...
python3 ./.feature/scripts/task.py init-context "$TASK_DIR" <dev_type>

# Set branch and scope
python3 ./.feature/scripts/task.py set-branch "$TASK_DIR" feature/<name>
python3 ./.feature/scripts/task.py set-scope "$TASK_DIR" <scope>
```
...
```bash
python3 ./.feature/scripts/task.py validate "$TASK_DIR"
python3 ./.feature/scripts/multi_agent/start.py "$TASK_DIR" --platform opencode
```
...
| Command | Description |
|---------|-------------|
| `/feature:parallel` | Start Multi-Agent Pipeline (this command) |
| `/feature:start` | Start normal development mode (single process) |
| `/feature:record-session` | Record session progress |
| `/feature:finish-work` | Pre-completion checklist |
...
```bash
python3 ./.feature/scripts/multi_agent/status.py                    # Overview
python3 ./.feature/scripts/multi_agent/status.py --log <name>       # View log
python3 ./.feature/scripts/multi_agent/status.py --watch <name>     # Real-time monitoring
python3 ./.feature/scripts/multi_agent/cleanup.py <branch>          # Cleanup worktree
```
