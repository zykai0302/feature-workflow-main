...
Before following this command, read `_shared/workflow-context.md`. Reuse current spec and task context so the integration lands in the right guideline files without duplicate discovery work.

## Usage

```
/feature:integrate-skill <skill-name>
```

**Examples**:

```
/feature:integrate-skill frontend-design
/feature:integrate-skill mcp-builder
```
...
> 
> - Guidelines content -> Write to `.feature/spec/{target}/doc.md`
> - Code examples -> Place in `.feature/spec/{target}/examples/skills/<skill-name>/`
...
| -------------------------------------------------------- | ------------------------------------------ |
| UI/Frontend (`frontend-design`, `web-artifacts-builder`) | `.feature/spec/frontend/`                  |
| Backend/API (`mcp-builder`)                              | `.feature/spec/backend/`                   |
| Documentation (`doc-coauthoring`, `docx`, `pdf`)         | `.feature/` or create dedicated guidelines |
| Testing (`webapp-testing`)                               | `.feature/spec/frontend/` (E2E)            |
...
- **Integration target**: `.feature/spec/{target}/`
...
| Type           | Path                                                      |
| -------------- | --------------------------------------------------------- |
| Guidelines doc | `.feature/spec/{target}/doc.md` (section: `skill-<name>`) |
| Code examples  | `.feature/spec/{target}/examples/skills/<name>/`          |
| Index update   | `.feature/spec/{target}/index.md`                         |
...
```bash
/feature:create-command use-<skill-name> Use <skill-name> skill following project guidelines
```
...
| `doc-coauthoring` | `.feature/`        | N/A (documentation workflow only)  |
...
After the skill is integrated:

```text
Next command options:
- /feature:create-command
- /feature:finish-work
Run `/feature:create-command` manually if you also want a shortcut command for the integrated skill.
Otherwise run `/feature:finish-work` manually.
```
