...
Before following this command, read `_shared/workflow-context.md`. Reuse already-loaded task artifacts, findings, and spec context instead of re-reading unchanged files.
...
**Block Rule**:
If infra/cross-layer changed but the related spec is still abstract, do NOT finish. Run `/feature:update-spec` manually first.
...
```
Development Flow:
  Write code -> Test -> (/feature:impact optional) -> /feature:check -> /feature:review -> /feature:finish-work -> confirm checks succeed -> git commit -> /feature:compound -> /feature:record-session
                                                                                              |                               |
                                                                                       Ensure completeness              Record progress + archive task

Test-Driven Development Flow:
  /feature:write-plan -> /feature:write-testcase -> /feature:check-testcase -> /feature:executing-plans or /feature:subagent-work -> /feature:check -> /feature:review -> /feature:finish-work
...
- `/feature:impact` - Optional blast-radius check before major changes or before final verification
- `/feature:review` - Formal multi-dimension review
- `/feature:compound` - Distill learnings into spec / task / workspace docs
