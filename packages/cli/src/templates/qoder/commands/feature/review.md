...
Perform a stricter review pass than `/feature:check`.

Before following this command, read `_shared/workflow-context.md`. Reuse already-loaded PRD, plan, findings, progress, and relevant specs unless any of them changed.
...
4. Decide whether the task may proceed to `/feature:finish-work`.
...
If review does not pass, hand off back to the execution step that needs more work, usually `/feature:executing-plans` or `/feature:subagent-work`, and tell the user to run it manually.
