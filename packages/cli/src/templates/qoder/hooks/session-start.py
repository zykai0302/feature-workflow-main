...
# -*- coding: utf-8 -*-
"""
Session Start Hook - Inject structured context for CodeBuddy
"""
...
def _get_task_status(feature_dir: Path) -> str:
    """Check current task status and return structured status string."""
    current_task_file = feature_dir / ".current-task"
    ...
    elif task_ref.startswith(".feature/"):
        task_dir = feature_dir.parent / task_ref
    else:
        task_dir = feature_dir / "tasks" / task_ref
    ...
    if task_status == "completed":
        return f"Status: COMPLETED\nTask: {task_title}\nNext: Archive with `python3 ./.feature/scripts/task.py archive {task_dir.name}` or start a new task"
...
def main():
    ...
    codebuddy_dir = project_dir / ".qoder"
...
""")
    ...
    context_script = feature_dir / "scripts" / "get_context.py"
    output.write(run_script(context_script))
    output.write("\n</current-state>\n\n")

    output.write("<workflow>\n")
    workflow_content = read_file(feature_dir / "workflow.md", "No workflow.md found")
    ...
    start_md = read_file(
        codebuddy_dir / "commands" / "feature" / "start.md", "No start.md found"
    )
    output.write(start_md)
    output.write("\n</instructions>\n\n")

    # R2: Check task status and inject structured tag
    task_status = _get_task_status(feature_dir)
