...
name: check-testcase
description: |
  验证测试用例完整性的skill，根据产品类型验证测试用例覆盖度，识别缺失测试点，补充测试用例和任务文档。
  在/write-testcase后使用，读取task.json确认产品类型，根据产品类型加载对应checklist（PK/HY/XF）进行验证。
  过滤相关模块、执行模块验证、补充缺失测试用例、评估是否涉及新需求、生成checklist报告、更新任务文档。
  输出testcase_checkdetail.md，更新testcase.md、prd.md、implementation-plan.md、task_plan.md、findings.md、progress.md。
---
...
Use this skill after `/write-testcase`:
...
||--------------|----------------|
|| Screen Control (PK) | `reference/testcase_checklist_PK.md` |
|| Conference (HY) | `reference/testcase_checklist_HY.md` |
|| Info Publishing (XF) | `reference/testcase_checklist_XF.md` |
...
cat reference/testcase_checklist_<PK|HY|XF>.md
```
...
- /executing-plans (sequential implementation)
- /subagent-work (delegated implementation)
