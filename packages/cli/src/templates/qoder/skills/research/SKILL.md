...
name: research
description: |
  正式代码库研究skill，在需求明确后、编写实现计划之前执行专门的代码库调查。
  整合多个研究视角：仓库结构和实现模式、现有学习成果和任务记忆、git历史（当历史上下文重要时）、
  框架或依赖文档（当任务依赖它们时）、问题情报（当用户报告的痛点或重复bug影响范围时）。
  使用GitNexus MCP工具进行代码库研究，支持语义搜索、调用链分析、影响分析等。
  研究结果写入findings.md，task.json状态更新为"researching"。
---
...
Before following this skill, read `reference/workflow-context.md`. Reuse unchanged workflow, task, and spec context from the immediately preceding Feature step.
Also re-read `task_plan.md` first so the current goal and phase are back in attention before you start searching.

This skill integrates multiple research lenses into one Feature skill:
...
```text
/brainstorm -> /init-plan -> /research -> /before-dev -> /write-plan -> /write-testcase -> /check-testcase -> /executing-plans or /subagent-work
```

`brainstorm` may include light pre-research, but this skill is where formal reusable findings are written down.
