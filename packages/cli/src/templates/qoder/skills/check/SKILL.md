---
name: check
description: Development-phase quality check that verifies current implementation follows relevant Feature specs and test cases before formal review. Identifies changed files via git diff, loads applicable spec modules using get_context.py, runs validation commands (lint, typecheck, tests), reports violations and fixes them when possible, records findings in progress.md and findings.md. Essential step between implementation and formal review.
---
...
Before following this skill, read `reference/workflow-context.md`. Reuse already-loaded package discovery, specs, findings, test cases, and progress unless something changed.
