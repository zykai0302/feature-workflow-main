...
For complex or vague tasks, **automatically start the brainstorm process** — do NOT skip directly to implementation. Use `/feature-brainstorm`.
...
1. Relevant spec files in `.feature/spec/`
...
| `/feature-before-frontend-dev` | Read frontend guidelines |
| `/feature-before-backend-dev` | Read backend guidelines |
| `/feature-check-frontend` | Check frontend code |
| `/feature-check-backend` | Check backend code |
...
feature auto-detects your platform based on config directories. For Cursor users, ensure detection works correctly:
...
```bash
export feature_PLATFORM=cursor
```

Or prefix commands:

```bash
feature_PLATFORM=cursor python3 ./.feature/scripts/task.py list
```
