...
│  - .feature/spec/         - ..agents/skills/               │
│  - File-based configs     - JSONL context files             │
└─────────────────────────────────────────────────────────────┘
                              │
┌─────────────────────────────▼───────────────────────────────┐
│                  CLAUDE CODE ONLY                            │
│  - ..agents/skills/         - ..agents/skills/ hooks     │
...
ls -la .agents/skills/feature-local/
```
...
```bash
mkdir -p .agents/skills/feature-local
```

Then create `.agents/skills/feature-local/SKILL.md`:
...
#### /feature:my-command
- **File**: `.agents/skills/my-command.md`
...
┌─────────────────────────────────▼───────────────────────────────────┐
│                         SKILLS LAYER                                 │
│  .agents/skills/feature/*.md   (skill definitions)                   │
│  .agents/skills/*.md             (sub-agent definitions)            │
└─────────────────────────────────┬───────────────────────────────────┘
                                  │
┌─────────────────────────────────▼───────────────────────────────────┐
│                          HOOKS LAYER                                 │
...
1. Create `.agents/skills/my-command.md`
2. Update `feature-local` skill with the change

### Adding an Agent

1. Create `.agents/skills/my-agent.md` with YAML frontmatter
...
1. Edit the hook script in `.agents/skills/`
...
| `.agents/skills/` | Skill configuration |
