...
Context Source: .feature/.current-task points to task directory
...
def get_current_task(repo_root: str) -> str | None:
    """
    Read current task directory path from .feature/.current-task
...
def get_check_context(repo_root: str, task_dir: str) -> str:
    ...
    if check_entries:
        ...
        check_files = [
            (".codebuddy/commands/feature/finish-work.md", "Finish work checklist"),
            (".codebuddy/commands/feature/check-cross-layer.md", "Cross-layer check spec"),
            (".codebuddy/commands/feature/check-backend.md", "Backend check spec"),
            (".codebuddy/commands/feature/check-frontend.md", "Frontend check spec"),
        ]
...
def get_finish_context(repo_root: str, task_dir: str) -> str:
    ...
    if finish_entries:
        ...
        finish_work = read_file_content(
            repo_root, ".codebuddy/commands/feature/finish-work.md"
        )
        if finish_work:
            context_parts.append(
                f"=== .codebuddy/commands/feature/finish-work.md (Finish checklist) ===\n{finish_work}"
            )

    # 2. Spec update process (for active spec sync)
    update_spec = read_file_content(
        repo_root, ".codebuddy/commands/feature/update-spec.md"
    )
    if update_spec:
        context_parts.append(
            f"=== .codebuddy/commands/feature/update-spec.md (Spec update process) ===\n{update_spec}"
        )
