...
Current Task is specified by `.feature/.current-task` file, content is the relative path to task directory.

Task directory path format: `.feature/tasks/{MM}-{DD}-{name}/`
...
Read `.feature/.current-task` to get current task directory path:

```bash
TASK_DIR=$(cat .feature/.current-task)
# e.g.: .feature/tasks/02-03-my-feature
```
...
**CRITICAL**: You must execute ALL actions in `next_action` array sequentially without stopping. After one subagent completes, immediately proceed to the next action. Do NOT stop after each phase.

Example execution flow:
```
1. Read task.json → get next_action = [implement, check, finish, create-pr]
2. Call implement subagent
3. When implement completes → immediately call check subagent
4. When check completes → immediately call finish subagent  
5. When finish completes → immediately run create_pr.py
6. All done → update task.json status to "review"
```
...
```bash
python ./.feature/scripts/multi_agent/create_pr.py
```
...
After reading `task.json`, you must execute ALL actions in `next_action` array continuously:

```
// Step 1: Read task.json
Read .feature/.current-task → get TASK_DIR
Read ${TASK_DIR}/task.json → get next_action array

// Step 2: Execute ALL actions sequentially
for each action in next_action:
    if action == "implement":
        Task(subagent_name: "implement", prompt: "Implement the feature...")
        // Wait for completion, then continue immediately
    
    if action == "check":
        Task(subagent_name: "check", prompt: "Check code changes...")
        // Wait for completion, then continue immediately
    
    if action == "finish":
        Task(subagent_name: "check", prompt: "[finish] Execute final check...")
        // Wait for completion, then continue immediately
    
    if action == "create-pr":
        Bash: python ./.feature/scripts/multi_agent/create_pr.py
        // This is the final action

// Step 3: All done
Print completion message with PR URL
```

**DO NOT**:
- Stop after implement phase
- Wait for user confirmation between phases
- Skip any action in the array

**ALWAYS**:
- Execute all phases in one session
- Proceed immediately after each subagent completes
- Complete the entire pipeline from implement to create-pr
...
The Task tool returns a result message when the subagent completes.
...
4. **Use subagent_type parameter** - Call Task with `subagent_type: "implement"`
