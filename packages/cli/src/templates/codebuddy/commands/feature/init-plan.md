...
This command is the Feature adaptation of `planning-with-files`: the files live under `.feature/tasks/<task>/` instead of the project root, but the operating model is the same.

Before following the steps below, read `_shared/workflow-context.md`. Then read `_shared/reference.md`, `_shared/task_plan.md`, `_shared/findings.md`, and `_shared/progress.md` as the canonical reference for how the three files should be used.
...
Use `/feature:init-plan` when any of these are true:
...
This command initializes or restores:
...
- if no task or `prd.md` exists yet, create or seed them during this command

During execution:

- restore existing context before creating anything
- use the `_shared/*.md` files as structure references, not as literal project outputs
...
```bash
cat .codebuddy/commands/feature/_shared/task_plan.md
cat .codebuddy/commands/feature/_shared/findings.md
cat .codebuddy/commands/feature/_shared/progress.md
```
...
After `/feature:init-plan`, every later command should follow these rules:
...
- simple task: implement directly, then run `/feature:check`
- if codebase understanding is unclear: run `/feature:research`
- if you want injected spec/context before editing: run `/feature:before-dev`
- if the task has become substantial: continue with `/feature:research` -> `/feature:before-dev` -> `/feature:write-plan` -> `/feature:write-testcase` -> `/feature:check-testcase` (mandatory for complex tasks)
```

For a simple task, "implement directly" means stay in the current session and start editing code after `/feature:init-plan` completes. It is not another required slash command.
