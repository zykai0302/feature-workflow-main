...
Before following this command, read `_shared/workflow-context.md`. Reuse the current workflow and spec summary rather than repeating the same setup explanation multiple times.
...
**The Solution**: The `.feature/workspace/` system captures what happened in each session - what was done, what was learned, what problems were solved. The `/feature:start` command reads this history at session start, giving AI "artificial memory."
...
**The Solution**: The `.feature/spec/` directory contains project-specific guidelines. The `/before-*-dev` commands inject this specialized knowledge into AI context before coding starts.
...
**The Solution**: The `/check-*` commands re-verify code against guidelines AFTER writing, catching drift that occurred during development. The `/feature:finish-work` command does a final holistic review.
...
### /feature:start - Restore AI Memory
...
- Without /feature:start: AI is blind. It might work on wrong branch, conflict with others' work, or redo already-completed work.
- With /feature:start: AI knows project context, can continue where previous session left off, avoids conflicts.

---

### /feature:before-dev - Inject Specialized Knowledge
...
1. Discovers spec layers by reading spec indexes and reads relevant guidelines
...
### /feature:check - Combat Context Drift
...
### /feature:check-cross-layer - Multi-Dimension Verification
...
### /feature:finish-work - Holistic Pre-Commit Review
...
### /feature:record-session - Persist Memory for Future

**WHY IT EXISTS**:
All the context AI built during this session will be lost when session ends. The next session's `/feature:start` needs this information.
...
**[1/8] /feature:start** - AI needs project context before touching code
**[2/8] python3 ./.feature/scripts/task.py create "Fix bug" --slug fix-bug** - Track work for future reference
**[3/8] /feature:before-dev** - Inject project-specific development guidelines
**[4/8] Investigate and fix the bug** - Actual development work
**[5/8] /feature:check** - Re-verify code against guidelines
**[6/8] /feature:finish-work** - Holistic cross-layer review
**[7/8] Human tests and commits** - Human validates before code enters repo
**[8/8] /feature:record-session** - Persist memory for future sessions

### Example 2: Planning Session (No Code)

**[1/4] /feature:start** - Context needed even for non-coding work
**[2/4] python3 ./.feature/scripts/task.py create "Planning task" --slug planning-task** - Planning is valuable work
**[3/4] Review docs, create subtask list** - Actual planning work
**[4/4] /feature:record-session (with --summary)** - Planning decisions must be recorded

### Example 3: Code Review Fixes

**[1/6] /feature:start** - Resume context from previous session
**[2/6] /feature:before-dev** - Re-inject guidelines before fixes
**[3/6] Fix each CR issue** - Address feedback with guidelines in context
**[4/6] /feature:check** - Verify fixes did not introduce new issues
**[5/6] /feature:finish-work** - Document lessons from CR
**[6/6] Human commits, then /feature:record-session** - Preserve CR lessons

### Example 4: Large Refactoring

**[1/5] /feature:start** - Clear baseline before major changes
**[2/5] Plan phases** - Break into verifiable chunks
**[3/5] Execute phase by phase with /feature:check after each** - Incremental verification
**[4/5] /feature:finish-work** - Check if new patterns should be documented
**[5/5] Record with multiple commit hashes** - Link all commits to one feature

### Example 5: Debug Session

**[1/6] /feature:start** - See if this bug was investigated before
**[2/6] /feature:before-dev** - Guidelines might document known gotchas
**[3/6] Investigation** - Actual debugging work
**[4/6] /feature:check** - Verify debug changes do not break other things
**[5/6] /feature:finish-work** - Debug findings might need documentation
**[6/6] Human commits, then /feature:record-session** - Debug knowledge is valuable
...
4. **Record everything** - /feature:record-session persists memory.
...
Check if `.feature/spec/` contains empty templates or customized guidelines:

```bash
# Check if files are still empty templates (look for placeholder text)
grep -l "To be filled by the team" .feature/spec/backend/*.md 2>/dev/null | wc -l
grep -l "To be filled by the team" .feature/spec/frontend/*.md 2>/dev/null | wc -l
```
...
If guidelines are empty templates (contain "To be filled by the team"), this is the first time using Feature in this project.

Explain to the developer:

"I see that the development guidelines in `.feature/spec/` are still empty templates. This is normal for a new Feature setup!
...
For example, for `.feature/spec/backend/database-guidelines.md`:
...
I recommend reading through `.feature/spec/` to familiarize yourself with the team's coding standards."
...
```bash
python3 ./.feature/scripts/task.py create "Fill spec guidelines" --slug fill-spec-guidelines
```
...
"You're now onboarded to the Feature workflow system! Here's what we covered:
...
1. Run `/feature:record-session` to record this onboard session
2. [If guidelines empty] Start filling in `.feature/spec/` guidelines
...
Next command: /feature:start
Run it manually when the developer is ready to begin real task work.
```
