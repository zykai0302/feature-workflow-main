...
Before following this command, read `_shared/workflow-context.md`. Reuse already-loaded task artifacts, review results, and changed-file context unless they changed.
...
- `.feature/workspace/<developer>/` later via `/feature:record-session`
...
- `_shared/schema.yaml` — canonical frontmatter fields and enum values
- `_shared/yaml-schema.md` — quick reference for track selection and category mapping
- `_shared/resolution-template.md` — section structure for new solution documents

Treat `_shared/schema.yaml` as the source of truth if `_shared/yaml-schema.md` and your memory ever conflict.
...
- `<category>` comes from the `problem_type` to category mapping in `_shared/yaml-schema.md`
...
If task memory is missing, run `/feature:init-plan` first.
...
Use `_shared/schema.yaml` and `_shared/yaml-schema.md` to choose the track from `problem_type`.
...
3. Classify it:
   - bug track
   - knowledge track
4. Read `_shared/schema.yaml` to validate frontmatter fields and enum values.
5. Read `_shared/yaml-schema.md` to map `problem_type` to `.feature/solutions/<category>/`.
6. Read `_shared/resolution-template.md` and assemble the final markdown using the matching track template.
...
12. Prepare a concise session summary for `/feature:record-session`.
...
The final document should follow `_shared/resolution-template.md` and include YAML frontmatter that satisfies `_shared/schema.yaml`.
...
- [ ] Frontmatter validated against `_shared/schema.yaml`
- [ ] Category chosen using `_shared/yaml-schema.md`
- [ ] Body structure follows `_shared/resolution-template.md`
...
- [ ] Session summary ready for `/feature:record-session`
...
```
✓ Compound completed
Next command: /feature:record-session
Run it manually.
```
