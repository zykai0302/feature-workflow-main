...
Triggered from `/feature:start` when the user describes a development task, especially when:
...
```bash
TASK_DIR=$(python3 ./.feature/scripts/task.py create "brainstorm: <short goal>" --slug <auto>)
```
...
```bash
# Create child tasks
CHILD1=$(python3 ./.feature/scripts/task.py create "Child task 1" --slug child1 --parent "$TASK_DIR")
CHILD2=$(python3 ./.feature/scripts/task.py create "Child task 2" --slug child2 --parent "$TASK_DIR")

# Or link existing tasks
python3 ./.feature/scripts/task.py add-subtask "$TASK_DIR" "$CHILD_DIR"
```
...
Task Workflow Phase 2.5 (Test Case Generation, mandatory for complex tasks)
  /feature:write-testcase → Generate test cases
...
| Command                | When to Use                                                                                      |
| ---------------------- | ------------------------------------------------------------------------------------------------ |
| `/feature:start`       | Entry point that triggers brainstorm                                                             |
| `/feature:research`    | Formal codebase research using GitNexus and other tools (call before asking technical questions) |
| `/feature:init-plan`   | Initialize task memory files after PRD is confirmed                                              |
| `/feature:finish-work` | After implementation is complete                                                                 |
| `/feature:update-spec` | If new patterns emerge during work                                                               |
