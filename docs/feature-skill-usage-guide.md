...
| 场景 | 命令 | 说明 |
|------|------|------|
| 开始新任务 | `/start-task` | 初始化会话，任务分类 |
| 需求澄清 | `/brainstorm` | 复杂任务需求分析 |
| 创建计划 | `/init-plan` | 创建任务文档 |
| 代码调研 | `/research` | 了解代码库 |
| 开发准备 | `/before-dev` | 加载开发规范 |
| 编写计划 | `/write-plan` | 生成实施计划 |
| 编写测试 | `/write-testcase` | 生成测试用例 |
| 验证测试 | `/check-testcase` | 验证测试完整性 |
| 执行计划 | `/executing-plans` | 顺序执行 |
| 开发检查 | `/check` | 代码自检 |
| 完成收尾 | `/finish-work` | 交付物校验 |
| 知识沉淀 | `/compound` | 创建解决方案文档 |
| 归档任务 | `/record-session` | 记录会话归档 |

#### 💡 最佳实践：多模型协作

建议使用不同模型进行开发与评审，获得更客观的质量保障：

| 阶段 | 推荐模型 | 说明 |
|------|----------|------|
| **开发阶段** | GLM-5.0 | 代码编写、需求实现 |
| **评审阶段** | Kimi-K2.5 | 独立评审、交叉验证 |

**进阶技巧**：
- 模型越强大，评审质量越高，交叉验证效果越好
- 可使用 **CodeBuddy + Lingma** 双工具协作，同时打开同一工程
  - CodeBuddy 负责开发执行
  - Lingma 负责独立评审
  - 实现真正的"双盲"质量把控
...
**💡 最佳实践 Tips**：
建议在开发自检后切换不同的模型进行正式评审，以获得更客观的质量保障：
- 自检可由开发模型完成，确保基础质量
- 正式评审建议使用不同的模型，获得更客观的视角
- 推荐组合：GLM-5.0 开发 + Kimi-K2.5 评审
...
**💡 最佳实践 Tips**：
推荐使用不同模型进行开发与评审，获得更客观的质量保障：
- **开发阶段**：使用 GLM-5.0 等模型进行代码编写
- **评审阶段**：使用 Kimi-K2.5 等不同模型进行独立评审
- 模型越强大，评审质量越高，交叉验证效果越好
- 进阶技巧：CodeBuddy + Lingma 双工具协作，同时打开同一工程，一个负责开发，一个负责评审
