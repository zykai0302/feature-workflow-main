<p align="center">
<picture>
<source srcset="assets/feature.png" media="(prefers-color-scheme: dark)">
<source srcset="assets/feature.png" media="(prefers-color-scheme: light)">
<img src="assets/feature.png" alt="feature Logo" width="500" style="image-rendering: -webkit-optimize-contrast; image-rendering: crisp-edges;">
</picture>
</p>
...
<a href="https://docs.tryfeature.app/">Docs</a> •
<a href="https://docs.tryfeature.app/guide/ch02-quick-start">Quick Start</a> •
<a href="https://docs.tryfeature.app/guide/ch13-multi-platform">Supported Platforms</a> •
<a href="https://docs.tryfeature.app/guide/ch08-real-world">Use Cases</a>
</p>

<p align="center">
<a href="https://www.npmjs.com/package/@uniview/feature"><img src="https://img.shields.io/npm/v/@uniview/feature.svg?style=flat-square&color=2563eb" alt="npm version" /></a>
<a href="https://github.com/uniview-ai/Feature/blob/main/LICENSE"><img src="https://img.shields.io/badge/license-AGPL--3.0-16a34a.svg?style=flat-square" alt="license" /></a>
<a href="https://github.com/uniview-ai/Feature/stargazers"><img src="https://img.shields.io/github/stars/uniview-ai/Feature?style=flat-square&color=eab308" alt="stars" /></a>
<a href="https://docs.tryfeature.app/"><img src="https://img.shields.io/badge/docs-tryfeature.app-0f766e?style=flat-square" alt="docs" /></a>
<a href="https://discord.com/invite/tWcCZ3aRHc"><img src="https://img.shields.io/badge/Discord-Join-5865F2?style=flat-square&logo=discord&logoColor=white" alt="Discord" /></a>
</p>

# Feature Skills 使用指导文档
...
本工作流吸收以下 5 个工具的优势，形成一套统一的 Skill 化工作流：
...
```
   ...
   └─────────┘       └─────────┘       └─────────┘
        ↓                  ↓                  ↓
      结束            /init-plan        /brainstorm
                  → 直接实现 → check   → /research
                  → finish-work        → /write-plan
...
| 阶段 | Skill | 作用 |
|------|-------|------|
| **启动** | `/start-task` | 初始化会话，任务分类 |
| **澄清** | `/brainstorm` | 需求澄清，生成 PRD |
| **初始化** | `/init-plan` | 创建任务目录和三文件 |
| **研究** | `/research` | 代码库调研 |
| **准备** | `/before-dev` | 加载开发规范 |
| **计划** | `/write-plan` | 生成实施计划 |
| **测试用例** | `/write-testcase` | 生成测试用例 |
| **测试验证** | `/check-testcase` | 验证测试完整性 |
| **执行** | `/executing-plans` | 顺序执行计划 |
| **执行** | `/subagent-work` | 子代理执行 |
| **分析** | `/impact` | 影响范围分析 |
| **检查** | `/check` | 开发自检 |
| **评审** | `/review` | 正式多维度评审 |
| **收尾** | `/finish-work` | 交付物校验 |
| **沉淀** | `/compound` | 创建解决方案文档 |
| **归档** | `/record-session` | 记录会话，归档任务 |

### 典型工作流

**简单任务流程**：
```
/start-task → /init-plan → 直接实现 → /check → /finish-work → /record-session
```

**复杂任务流程**：
```
/start-task → /brainstorm → /init-plan → /research → /before-dev 
→ /write-plan → /write-testcase → /check-testcase 
→ /executing-plans | /subagent-work → /impact(可选) 
→ /check → /review → /finish-work → /compound → /record-session
```
...
- [2. Feature CLI 安装](#2-feature-cli-安装)
  - [2.1 从公司外网安装](#21-从公司外网安装)
  - [2.2 从公司内网安装](#22-从公司内网安装)
- [3. CodeBuddy 平台工作流程](#3-codebuddy-平台工作流程)
  - [3.1 工作流概述](#31-工作流概述)
  - [3.2 Skills 详解与流程图](#32-skills-详解与流程图)
- [4. 任务处理流程](#4-任务处理流程)
...
```bash
# 克隆公司内网仓库
git clone -b feature1 --single-branch http://igcode.uniview.com/rd-dis/rd_tools/myagent.git
...
Feature 在 CodeBuddy 平台上提供了完整的 AI 辅助开发工作流，通过一系列 Skills 来管理开发过程的各个阶段。核心目录结构如下：
...
#### 3.2.1 `/start-task` - 会话启动

**作用**：初始化会话，读取项目上下文，判断任务类型，路由到后续 Skill。
...
```mermaid
flowchart TD
    A["用户发起 /start-task"] --> B[读取 workflow.md]
    ...
    F -->|简单问题| G[直接回答或轻量修改]
    F -->|简单任务| H[路由到 /init-plan]
    F -->|复杂任务| I[路由到 /brainstorm]
...
**输入输出**：
- 输入：用户需求描述
- 输出：任务类型判断，以及下一条需要手动运行的 Skill
- 状态更新：无强制文件写入；任务文件通常在后续 `/init-plan` 或 `/brainstorm` 中创建/更新

---

#### 3.2.2 `/brainstorm` - 需求澄清
...
```mermaid
flowchart TD
    A["用户发起 /init-plan"] --> B[检查 PRD 是否存在]
    B --> C{三文件是否存在}
...
```mermaid
flowchart TD
    A["用户发起 /research"] --> B[检查 GitNexus index]
    B --> C{索引是否可用}
...
flowchart TD
    ...
    O -->|是| P[进入 /write-testcase]
    O -->|否| Q[进入执行阶段]
    
    M -->|不准确| R[返回 /brainstorm]
...
#### 3.2.6.1 `/write-testcase` - 生成测试用例（复杂任务必须）
...
```mermaid
flowchart TD
    A["用户发起 /write-testcase"] --> B[读取任务文档]
    B --> C[读取测试规范和模板]
    C --> D[识别产品类型]
...
#### 3.2.6.2 `/check-testcase` - 验证测试用例完整性（复杂任务必须）
...
```mermaid
flowchart TD
    A["用户发起 /check-testcase"] --> B[读取任务和测试文档]
    B --> C[确认产品类型]
...
```mermaid
flowchart TD
    A["用户发起 /executing-plans"] --> B[读取 implementation-plan.md]
    B --> C[解析任务列表]
...
```mermaid
flowchart TD
    A["用户发起 /subagent-work"] --> B[读取 implementation-plan.md]
    B --> C[解析任务列表]
...
```mermaid
flowchart TD
    A["用户发起 /finish-work"] --> B[检查 task_plan.md]
    B --> C{所有 phase 是否完成}
...
```mermaid
flowchart TD
    A["用户发起 /record-session"] --> B[记录 commit 信息]
    B --> C[记录 session 标题]
    C --> D[记录 session 摘要]
...
flowchart TD
    ...
    L -->|是| N["调用 /finish-work"]
    N --> O[确认所有检查项]

    O --> P["调用 /record-session"]
    P --> Q[记录会话并归档]
    Q --> R[任务完成]
```
...
**步骤**：
1. 运行 `/start-task`，描述需求："添加日志工具函数"
2. Feature 判断为简单任务
3. 运行 `/init-plan`，创建或恢复任务目录和基础文件（必要时补齐 `task.json`、`prd.md`、`progress.md`、`task_plan.md`、`findings.md`）
4. 读取 `spec/utils/index.md` 了解规范要求
5. **直接实施代码修改**：在 `src/utils/logger.ts` 添加日志函数
6. 编写单元测试
7. 运行测试验证功能
8. 运行 `/check`，验证代码质量和规范对齐
9. 运行 `/finish-work`，确认所有检查项通过
10. 运行 `/record-session`，记录工作内容并归档任务

**关键说明**：
- 简单任务在 `init-plan` 后默认直接写代码，不需要默认执行 `/write-plan`
- `/init-plan` 负责兜住轻量流程入口：如果任务目录或 `prd.md` 不存在，应先补齐再进入编码
- 这里的"直接写代码"不是再运行一个新的 Skill，而是在当前会话里继续实现任务
- 如果实现前还需要加载规范或注入上下文，可先运行 `/before-dev`
- 如果任务范围扩大、实现路径不清晰，升级为 `/research` -> `/before-dev` -> `/write-plan`
...
**步骤**：
1. 运行 `/start-task`，描述问题："用户登录响应时间超过5秒"
2. 运行 `/research`，分析登录流程，定位性能瓶颈
3. 在 `findings.md` 记录：数据库查询缺少索引、N+1 查询问题
4. 设计修复方案：添加数据库索引、优化查询逻辑
5. 运行 `/write-plan`，编写修复计划
6. 运行 `/executing-plans`，实施修复
7. 运行测试验证，确认响应时间降到 500ms 以内
8. 运行 `/check`，验证代码质量
9. 运行 `/compound`，沉淀经验到规范文档
10. 运行 `/finish-work`，确认完成
11. 运行 `/record-session`，归档任务
...
flowchart TD
    ...
    M -->|通过| O[调用 /write-testcase]
    O --> P[生成 testcase.md]
    P --> Q[调用 /check-testcase]
    Q --> R{测试用例验证}
    ...
    T -->|顺序内联| U[调用 /executing-plans]
    T -->|子代理执行| V[调用 /subagent-work]
    ...
    AG -->|是| AJ[调用 /finish-work]
    AJ --> AK[调用 /compound]

    AK --> AL[调用 /record-session]
    AL --> AM[任务完成]
```
...
**第一阶段：需求澄清**
1. 运行 `/start-task`，描述需求："实现用户认证和授权系统"
2. 运行 `/brainstorm`，进行需求澄清
   - 调研现有系统：查询相关代码、已有实现
...
**第二阶段：初始化和研究**
5. 运行 `/init-plan`，创建三文件工作流
6. 运行 `/research`，进行代码调研
   - 分析现有用户模型和数据库结构
   - 研究现有权限系统实现
   - 查找相关依赖和库
   - 评估技术选型（JWT 库、加密库等）
...
13. 运行 `/check-testcase`，验证测试用例完整性
...
| Skill                    | 阶段   | 主要作用                 | 关键输出文件                                                 |
| ----------------------- | ---- | -------------------- | ------------------------------------------------------ |
| `/start-task`           | 启动   | 初始化会话，判断任务类型并给出下一条 Skill | 任务分类结果                                                 |
| `/brainstorm`           | 澄清   | 需求澄清，生成 PRD          | `prd.md`                                               |
| `/init-plan`            | 初始化  | 创建或恢复轻量任务入口所需文件      | `prd.md`, `task_plan.md`, `findings.md`, `progress.md` |
| `/research`             | 研究   | 代码调研，建立事实基线          | `findings.md` (更新)                                     |
| `/before-dev`           | 准备   | 加载规范，准备开发            | 上下文就绪状态                                                |
| `/write-plan`           | 计划   | 生成实施计划               | `implementation-plan.md`                               |
| `/write-testcase`       | 测试用例 | 生成功能测试用例（复杂任务必须）     | `testcase_analysis.md`, `testcase.md`                  |
| `/check-testcase`       | 测试验证 | 验证测试用例完整性（复杂任务必须）    | `testcase_checkdetail.md`                              |
| `/executing-plans`      | 执行   | 顺序执行计划               | 代码, `progress.md`                                      |
| `/subagent-work`        | 执行   | 子代理执行任务              | 代码, `progress.md`                                      |
| `/impact`               | 分析   | 影响面分析                | 影响报告                                                   |
| `/check`                | 自检   | 开发中质量检查              | 检查结果                                                   |
| `/review`               | 评审   | 正式多角色评审              | 评审报告                                                   |
| `/finish-work`          | 收尾   | 汇总完成条件               | 完成确认                                                   |
| `/compound`             | 沉淀   | 创建结构化解决方案文档          | `solutions/<category>/<filename>.md`                   |
| `/record-session`       | 归档   | 记录会话，归档任务            | `journal-N.md`, 归档目录                                   |
...
A: Feature 会在 `/start-task` 阶段根据以下标准判断：
...
**Q4: `/check` 和 `/review` 有什么区别？**

A: 
- `/check`：轻量自检，面向开发中场景，快速验证代码质量
- `/review`：正式评审，多角色、多维度深度检查，面向提交前场景

**Q5: 如何恢复中断的任务？**

A: 运行 `/resume-plan` Skill，它会：
